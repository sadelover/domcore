...
uint32_t conver_add_to_index( uint32_t device_id,BACNET_ADDRESS * src ,int mode)
{
	if(mode == 0)
	{
		return device_id;
	...
	else if(mode == 2)
	{
		if(src)
		{
			char sadr_string[20]={0};
			sprintf(sadr_string, "%d.%d.%d.%d", (int)src->mac[0],(int)src->mac[1], (int)src->mac[2], (int)src->mac[3]);
			return inet_addr(sadr_string) + device_id*1000000000000;
		}
	}
	return device_id;
}
    ...
    BACNET_ADDRESS * src)
{
	...
	//device_id = conver_add_to_index(device_id,src,g_nReadMode);
