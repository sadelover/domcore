...
{       /* number of milliseconds to wait for a packet */
    ...
    if ((pdu[1] == BVLC_ORIGINAL_UNICAST_NPDU) ||
        ...
        if ((sin.sin_addr.s_addr == BIP_Address.s_addr) &&
            ...
            pdu_len = 0;
#if PRINT_ENABLED
            fprintf(stderr, "ERROR: BIP: src is me. Discarded!\n");
#endif
        } else {
...
#if PRINT_ENABLED
        fprintf(stderr, "ERROR: BIP: BVLC discarded!\n");
#endif
    }
...
void bip_set_addr_mask( uint32_t net_address )
{
	BIP_Address_Mask = net_address;
}

uint32_t bip_get_addr_mask( void )
{
	return BIP_Address_Mask;
}
