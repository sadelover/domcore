    ...
    uint32_t net_address)
{
	...
	bip_set_addr_mask(net_mask);
    if (BIP_Debug) {
        struct in_addr address;
        address.s_addr = net_mask;
        printf("Bacnet IP Mask: %s\n", inet_ntoa(address));
    }
    broadcast_address = (net_address & net_mask) | (~net_mask);
	//broadcast_address = 0xFFFFFFFF; //golding add manually.
    ...
    char *ifname)
{
    ...
    if (BIP_Debug) {
        fprintf(stderr, "INFO : Bacnet IP Address: %s\n", inet_ntoa(address));
    }
    ...
    if (BIP_Debug) {
        ...
        fprintf(stderr, "INFO : Bacnet IP Broadcast Address: %s\n",
            inet_ntoa(broadcast_address));
        fprintf(stderr, "INFO : Bacnet UDP Port: 0x%04X [%hu]\n", ntohs(bip_get_port()),
            ntohs(bip_get_port()));
    }
    ...
    if (sock_fd < 0) {
        fprintf(stderr, "ERROR: bip: failed to allocate a socket.\n");
        return false;
    }
    ...
    rv = setsockopt(sock_fd, SOL_SOCKET, SO_REUSEADDR, (char *) &value,
        sizeof(value));
    if (rv < 0) {
        fprintf(stderr, "ERROR: bip: failed to set REUSEADDR socket option.\n");
        close(sock_fd);
        bip_set_socket(-1);
        return false;
    }
    ...
    rv = setsockopt(sock_fd, SOL_SOCKET, SO_BROADCAST, (char *) &value,
        sizeof(value));
    if (rv < 0) {
        fprintf(stderr, "ERROR: bip: failed to set BROADCAST socket option.\n");
        close(sock_fd);
        bip_set_socket(-1);
        return false;
    }
    ...
    rv = setsockopt(sock_fd, SOL_SOCKET, SO_REUSEPORT, (char *) &value,
        sizeof(value));
    if (rv < 0) {
        fprintf(stderr, "ERROR: bip: failed to set REUSEPORT socket option.\n");
        close(sock_fd);
        bip_set_socket(-1);
        return false;
    }
    ...
    if (rv < 0) {
        fprintf(stderr, "ERROR: bip: failed to bind to %s port %hu\n",
