...
bool CDatabaseSeesion::Connect(Database_Param *pDbParam)
{
	...
	while(repeattime--)
	{
			...
			, pDbParam->client_flag))
		{
			...
			if(strErr.find("Access denied")>=0)
			{
				pDbParam->user = "root";
				pDbParam->password = "RNB.beop-2013";
			}
			// log the error.
...
bool CDatabaseSeesion::AddMysqlErrLog( string strOperation, string strErrInfo )
{
	...
	if(Project::Tools::FindOrCreateFolder(strFilePath))
	{
		...
		GetLocalTime(&stNow);
		CString strLogPath;
		strLogPath.Format(_T("%s\\core_err_%d_%02d_%02d_Hour%02d.log"),strFilePath.c_str(),stNow.wYear,stNow.wMonth,stNow.wDay, stNow.wHour);
		CStdioFile	ff;
		if(ff.Open(strLogPath,CFile::modeCreate|CFile::modeNoTruncate|CFile::modeWrite))
		{
			...
			CString strLog;
			strLog.Format(_T("%s::%s(%s)...\r\n"),strTime.c_str(),strOp.c_str(),strEi.c_str());
			ff.Seek(0,CFile::end);
...
bool CDatabaseSeesion::AddMysqlErrSqlLog( string strOperation, string strErrInfo, string strErrSql )
{
	...
	if(Project::Tools::FindOrCreateFolder(strFilePath))
	{
		...
		if(ff.Open(strLogPath,CFile::modeCreate|CFile::modeNoTruncate|CFile::modeWrite))
		{
			...
			CString strLog;
			strLog.Format(_T("%s::%s(%s)(%s)...\r\n"),strTime.c_str(),strOp.c_str(),strEi.c_str(),strSql.c_str());
			ff.Seek(0,CFile::end);
			ff.WriteString(strLog);
			ff.Close();
			return true;
