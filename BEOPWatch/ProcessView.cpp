	...
	if (FindProcessByName_other(exefile))
	{
			...
			if (NULL != hProcess)
			{
				...
				if (GetProcessMemoryInfo(hProcess, &pmc, sizeof(pmc)))
				{
					...
					TRACE("[info] Main memory usage, Currently used memory：%.4fM，Max used：%.4fM\r", fCurUse, fMaxUse);
					CString str;
					str.Format(_T("%s Currently used memory：%.4fM，Max used：%.4fM\r"),exefile,fCurUse, fMaxUse);
