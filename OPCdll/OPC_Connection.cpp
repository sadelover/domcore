...
, m_bBrowseFlat( TRUE )
, m_isconnected(false)
, m_bAddItemSuccess(false)
, m_bAddGroupSuccess(false)
{
							   ...
							   , int nBufLen)	// [in]
{
	...
	case VT_R4:
		strVal.Format (_T("%f"), vtVal.fltVal);
		break;
			...
			for (DWORD i=0; i<dwSize; i+=cbElements, lpByte+=cbElements)
			{
				...
				else if (cbElements == 4)
				{
					if (vtVal.vt ==	(VT_R4	| VT_ARRAY))
						StringCchPrintf (szNum, 32, _T("%f"), *(float *)lpByte);
...
bool COPCConnection::Connect(wstring strOPCServerIP,int nOPCClientIndex)
{
	...
	{		
		...
		{
			CString strLog;
			strLog.Format(_T("INFO : Connect to OPC Server :%s\r\n"),serverhost);
			PrintLog(strLog.GetString(),false);
		}
		else
		{	
			CString strOut;
			strOut.Format(_T("INFO : Connect to OPC Server :%s(index:%d)\r\n"),serverhost,nOPCClientIndex);
	...
	else
	{
		...
		{
			PrintLog(L"ERROR: Can not find OPC Service...\r\n");
			CDebugLog::GetInstance()->SetErrCode(ERROR_CUSTOM_OPCSERVICE_UN_FIND);
		}
	}
...
bool COPCConnection::AddGroup(int nUpdateRate,int nLanguageID)
{
	if (NULL == m_pGroup){
	...
	m_pGroup->SetLanguageID(nLanguageID);
	m_pGroup->SetDeadband(0);
	m_pGroup->SetBias(0);
	m_pGroup->SetUpdateRate(nUpdateRate);
	m_pGroup->SetUpdateMethod(2);

	return m_pServer->AddGroup(m_pGroup);
}

bool COPCConnection::AddItems(int nMultilAddItems)
{
	int			 nPos	= 0;
	HRESULT		 hr		= 0;
...
bool COPCConnection::GetItemValue(const wstring& strItemID, wstring &strValue, bool bFromDevice)
{
	...
	if (NULL == pItem){
		//没有找到此节点
		CDebugLog::GetInstance()->SetErrCode(ERROR_CUSTOM_OPCITEM_UNEXIST);
		return false;
	}
	...
	if(pItempPro==NULL)
	{
		CDebugLog::GetInstance()->SetErrCode(ERROR_CUSTOM_OPCITEM_PROPERTY_NULL);
		return false;
	}
...
bool COPCConnection::ReConnect()
{
	...
	if(m_pServer)
	{
		...
		CLSID clsid;
		if(GetCLSIDByProgID(m_strserverhost,m_strserverprogid,clsid))
		{
			if(m_strserverhost == L"localhost" || m_strserverhost == L"127.0.0.1")
			{
				m_isconnected = m_pServer->Start(false,clsid);
			}
			else
			{
				m_isconnected = m_pServer->Start(true,clsid);
			}
		}
		return m_isconnected;
	}
...
bool COPCConnection::GetItemValueBuffer( const wstring& strItemName, wstring &strValue )
{
	CKItem *pItem = FindItem(strItemName);
	if (pItem)
	{
...
bool COPCConnection::GetUpdateTime20( COleDateTime& oleUpdate ,bool& bUpdateTimeAsync20, bool bOutAsync20)
{
	if(m_pGroup)
	{
		return m_pGroup->GetUpdateTime20(oleUpdate,bUpdateTimeAsync20,bOutAsync20);
	}
	return false;
}

void COPCConnection::PrintLog( const wstring &strLog,bool bSaveLog /*= true*/ )
{
