...
{
	m_oleUpdateTimeAsync20 = COleDateTime::GetCurrentTime();
	m_bUpdateTimeAsync20 = false;
	m_bOutAsync20 = false;
	m_bAsync20Mode = false;
	m_bAsync20First = false;
	m_bSyncFirst = false;
	m_mapAsync20ItemError.clear();
	m_mapSyncItemError.clear();
	m_mapAsync20ItemWrite.clear();
}
										 ...
										 HRESULT *pErrors)
{
	...
	if (dwTransID != 0)
	{
		_tprintf(L"INFO : OPC OnDataChange2.0 Update:%u(TransID:%u) \r\n",dwCount,dwTransID);
	}
	else
	{
		if(m_bOutAsync20)
	...
	for (DWORD dwItem = 0; dwItem < dwCount; dwItem++)
	{
		...
		try
		{
			...
			if(pErrors && SUCCEEDED(pErrors[dwItem]))
			{
			...
			{
				if(m_bAsync20Mode)			//异步模式
				{
					_tprintf(L"ERROR : OPC OnDataChange2.0 item '%s' fail(0x%x).\r\n",pItem->GetItemID (), pErrors [dwItem]);
					if(dwTransID!=0)
					{
						if(!m_bAsync20First)						//只重新添加一次
						{
							pItem->SetActive(FALSE);
							m_mapAsync20ItemError[pItem] = 1;
							m_bAsync20First = true;
						}
					}			
				}						//同步读取
				else
		...
		{
			// Must have tried to use a bad pItem pointer.  Bump a bad item  handle count and try to process then next item:
			++cdwBadItemHandles;
			continue;
		}
	}
	...
	if(bResultSuccess)
	{
		m_bUpdateTimeAsync20 = true;
		m_oleUpdateTimeAsync20 = COleDateTime::GetCurrentTime();
	}

	if (cdwBadItemHandles)
	{
		_tprintf(L"ERROR : OPC OnDataChange2.0 bad item handles received %d.\r\n",cdwBadItemHandles);
	}
											...
											HRESULT *pErrors)
{
	...
	for (DWORD dwItem = 0; dwItem < dwCount; dwItem++)
	{
		try
		{
			...
			if(pErrors && SUCCEEDED(pErrors[dwItem]))
			{
				m_mapAsync20ItemWrite[pItem] = 1;
				bResultSuccess = true;
			}
			else if (pErrors && FAILED (pErrors [dwItem]))				//有错误
			{
...
bool IKDataSink20::GetUpdateTime20( COleDateTime& oleUpdate,bool& bUpdateTimeAsync20,bool bOutAsync20/*=true*/)
{
	oleUpdate = m_oleUpdateTimeAsync20;
	bUpdateTimeAsync20 = m_bUpdateTimeAsync20;
	m_bOutAsync20 = bOutAsync20;
	m_bAsync20Mode = true;
	return true;
}

bool IKDataSink20::GetAsync20ErrorItem( CObArray &cItemList )
{
	cItemList.RemoveAll();
	if(m_mapAsync20ItemError.size() <= 0)
		return false;
	if(m_bAsync20Mode)
	{
		hash_map<CKItem*,int>::iterator iter = m_mapAsync20ItemError.begin();
		while(iter != m_mapAsync20ItemError.end())
		{
...
bool IKDataSink20::GetSync20ErrorItem( CObArray &cItemList )
{
	if(!m_bSyncFirst)
	{
		cItemList.RemoveAll();
		if(m_mapSyncItemError.size() <= 0)
			return false;

		m_bSyncFirst = true;
		hash_map<CKItem*,int>::iterator iter = m_mapSyncItemError.begin();
		while(iter != m_mapSyncItemError.end())
		{
			cItemList.Add((*iter).first);
			iter++;
		}
...
bool IKDataSink20::GetAsync20WriteItem( CObArray &cItemList )
{
	cItemList.RemoveAll();
	if(m_mapAsync20ItemWrite.size() <= 0)
		return false;

	hash_map<CKItem*,int>::iterator iter = m_mapAsync20ItemWrite.begin();
	while(iter != m_mapAsync20ItemWrite.end())
	{
		cItemList.Add((*iter).first);
		iter++;
	}
