...
public:
	COleDateTime m_oleUpdateTimeAsync20;			//异步刷新时间
	bool	m_bUpdateTimeAsync20;
	bool	m_bOutAsync20;							
	bool	m_bAsync20Mode;	//异步读写模式
	bool	m_bAsync20First;		//异步读取出错只添加一次
	bool	m_bSyncFirst;			//同步读取出错 添加一次
	hash_map<CKItem*,int>		m_mapAsync20ItemError;		//item 无效
	hash_map<CKItem*,int>		m_mapSyncItemError;		//item 无效
	hash_map<CKItem*,int>		m_mapAsync20ItemWrite;	//异步写成功值
private:
	DWORD m_cnRef;
};
