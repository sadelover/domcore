...
//  void
// **************************************************************************
bool CKServer::AddGroup (CKGroup *pGroup, bool bLoadingProject /*= false */)
{
	bool bAddGroupSuccess = false;
	ASSERT (pGroup != NULL);
	...
	if (m_bConnected)
	{
		...
		if (SUCCEEDED (hr))
		{
			...
			{
				_tprintf(L"ERROR: OPC AddGroup %s Fail(IUnknown is invalid).\r\n",pGroup->GetName ());
				bAddGroupSuccess = false;
			}
		}
		...
		else
		{
			...
			HRESULT hr1 = GetIServer()->GetErrorString(hr,0,&pErrorString);
			if(SUCCEEDED(hr1))
			{
				CString strOut;
				strOut.Format(_T("ERROR: OPCServer Can't AddGroup(%s)\r\n"),pErrorString);			
				PrintLog(strOut,false);
			}
			else
			{
				CString strOut;
				strOut.Format(_T("ERROR: OPCServer Can't AddGroup(0x%x)\r\n"),hr);			
				PrintLog(strOut,false);
			}
			bAddGroupSuccess = false;
		}
	...
	else
	{
		...
		CString strOut = _T("ERROR: OPCServer Can't AddGroup(Connect Fail)\r\n");		
		PrintLog(strOut,false);
		bAddGroupSuccess = false;
...
bool CKServer::Start( bool bRemote,CLSID clsid )
{
	// Attempt to connect to the OPC Server:
	bool bStartSuccess = false;
	if(bRemote)
	{
		bStartSuccess = ConnectByCLSID(clsid);
	}
	else
	{
		bStartSuccess = Connect ();
	}
...
bool CKServer::ConnectByCLSID( CLSID clsid1 )
{
	// Program ID of OPC Server should have been defined by now:
	ASSERT (!m_strProgID.IsEmpty ());
...
void CKServer::PrintLog( const CString &strLog,bool bSaveLog /*= true*/ )
{
	_tprintf(strLog);
}
