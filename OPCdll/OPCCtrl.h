...
public:
	 ...
	 */
	virtual bool Init(wstring strOPCServerIP,bool bDisableCheckQuality=false,int nOPCClientIndex = -1,int nUpdateRate = 500, int nLanguageID=0, int nMultilAddItems =1000, int nDebugOPC=0);
	bool	Exit();	
	bool	ReConnectOPC();
	...
	void	ClearPointList();
	void	OutDebugInfo(const wstring strOut);
	bool	ReloadErrorItemsHandle();
	bool	ActiveReloadSynErrorItems();
	int		GetPosition(const wstring shortname);
	void 	PrintLog(const wstring &strLog,bool bSaveLog = true);

	bool	ReReadAsync20Item(CObArray &cAsync20WriteItemList);			//重新读取异步写值点
public:
	bool	m_bOPCConnect;
	...
	bool	m_bReloadSynErrorItem;		//添加同步出错点
public:
...
protected:
	...
	bool	ReLoadItemsHandle(vector<wstring> vecItemID,bool bAsync20=false);
...
public:
	...
	virtual bool AddItems(int nMultiAdd = 1000);
